...
app.layout = html.Div(
    [
        html.H2(
            "Régression linéaire + Histogramme interactif",
...
def update_graph(clickData, stored_x):
    ...
    fig = make_subplots(
        ...
        subplot_titles=("Données et régression", "Histogramme de X (cliquer)"),
    )
    ...
    # Nuage
    fig.add_trace(
        go.Scatter(x=X.flatten(), y=Y, mode="markers", name="Données"), row=1, col=1
    )

    # Droite
    fig.add_trace(
        go.Scatter(x=x_line, y=y_line, mode="lines", name="Régression"), row=1, col=1
    )
    ...
    fig.add_trace(
        go.Histogram(x=X.flatten(), nbinsx=12, name="Distribution X", opacity=0.7),
        row=1,
        col=2,
    )

    # Ligne verticale sur X sélectionné
    fig.add_vline(x=x_value, line_dash="dash", line_color="red", row=1, col=2)
    ...
    fig.update_xaxes(title_text="Années d'expérience", row=1, col=1)

    fig.update_yaxes(title_text="Salaire", row=1, col=1)

    fig.update_xaxes(title_text="Années d'expérience", row=1, col=2)
    ...
    text = (
        f"Valeur sélectionnée : x = {x_value:.2f}  →  " f"Salaire prédit ≈ {y_pred:.2f}"
    )
